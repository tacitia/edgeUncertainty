...
for key1 in results:
	for key2 in results[key1]:
		results[key1][key2].append(['subject', 'strEnc', 'cerEnc', 'difficulty', 'targetParam', 'accuracy'])
		...
		with open(dir + filename, 'r') as infile:
			...
			for dataset in data:
				...
				for i in range(1, 289):
					...
					if answer == response:
						b_acc = 1
					results[targetType][task].append([subject, conditions[0], conditions[1], difficulty, targetParam, b_acc])	

with open('str_vs_results.csv', 'w') as outfile:
	writer = csv.writer(outfile, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
	for a in results['S']['vs']:
		writer.writerow(a)
		
with open('str_cp_results.csv', 'w') as outfile:
	writer = csv.writer(outfile, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
	for a in results['S']['cp']:
		writer.writerow(a)

with open('cer_vs_results.csv', 'w') as outfile:
	writer = csv.writer(outfile, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
	for a in results['C']['vs']:
		writer.writerow(a)

with open('cer_cp_results.csv', 'w') as outfile:
	writer = csv.writer(outfile, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
	for a in results['C']['cp']:
		writer.writerow(a)	
