...
//  	experimentr.release();
	...
	var updateNode = function() {
  	...
  	function createGraphSVG(svgName, data) {
		var svg = d3.select(".main").select(svgName)
			.attr("width", width)
			.attr("height", height);
	...
	function createLegend() {
		...
		var legend = d3.select('.main').append('svg').append('g')
			.attr('width', legendW)
			.attr('height', legendH)
  	...
  	function mainFunc() {
  		...
  		createGraphSVG('#svgA', dataA);
  		createGraphSVG('#svgB', dataB);
...
//  		createLegend();
  	...
  	$(document).ready(function() {
		d3.json(window.dataPathA, function(error, json) {
	...
	function loadDataB() {
		d3.json(window.dataPathB, function(error, json) {
		  if (error) return console.warn(error);
		  dataB = json;
		  dataReady();
		});
